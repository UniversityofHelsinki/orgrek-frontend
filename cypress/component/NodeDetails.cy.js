...
describe('NodeDetails.cy.js', () => {
  it('shows correct information when node is current in relation to selected day', () => {
    cy.intercept(
      'GET',
      /.*\/api\/node\/[0-9]+\/[0-9]+\.[0-9]+\.[0-9]+\/attributes/g,
      { fixture: '2022-06-21/current/attributes.json' }
    );
    cy.intercept(
      'GET',
      /.*\/api\/node\/parents\/[0-9]+\/[0-9]+\.[0-9]+\.[0-9]+/g,
      { fixture: '2022-06-21/current/parents.json' }
    );
    cy.intercept(
      'GET',
      /.*\/api\/node\/children\/[0-9]+\/[0-9]+\.[0-9]+\.[0-9]+/g,
      { fixture: '2022-06-21/current/children.json' }
    );
    cy.intercept(
      'GET',
      /.*\/api\/node\/predecessors\/[0-9]+\/[0-9]+\.[0-9]+\.[0-9]+/g,
      { fixture: '2022-06-21/current/predecessors.json' }
    );
    cy.intercept(
      'GET',
      /.*\/api\/node\/successors\/[0-9]+\/[0-9]+\.[0-9]+\.[0-9]+/g,
      { fixture: '2022-06-21/current/successors.json' }
    );
    cy.intercept(
      'GET',
      /.*\/api\/node\/historyandcurrent\/[0-9]+\/[0-9]+\.[0-9]+\.[0-9]+\/attributes/g,
      { fixture: '2022-06-21/current/historyAttributes.json' }
    );
    cy.intercept(
      'GET',
      /.*\/api\/node\/historyandcurrent\/parents\/[0-9]+\/[0-9]+\.[0-9]+\.[0-9]+/g,
      { fixture: '2022-06-21/current/historyParents.json' }
    );
    cy.intercept(
      'GET',
      /.*\/api\/node\/historyandcurrent\/children\/[0-9]+\/[0-9]+\.[0-9]+\.[0-9]+/g,
      { fixture: '2022-06-21/current/historyChildren.json' }
    );
    cy.intercept(
      'GET',
      /.*\/api\/node\/futureandcurrent\/[0-9]+\/[0-9]+\.[0-9]+\.[0-9]+\/attributes/g,
      { fixture: '2022-06-21/current/futureAttributes.json' }
    );
    cy.intercept(
      'GET',
      /.*\/api\/node\/futureandcurrent\/parents\/[0-9]+\/[0-9]+\.[0-9]+\.[0-9]+/g,
      { fixture: '2022-06-21/current/futureParents.json' }
    );
    cy.intercept(
      'GET',
      /.*\/api\/node\/futureandcurrent\/children\/[0-9]+\/[0-9]+\.[0-9]+\.[0-9]+/g,
      { fixture: '2022-06-21/current/futureChildren.json' }
    );

    const node = createNode(
      new Date('2020-01-01T00:00:00+00:00'),
      new Date('2030-01-01T00:00:00+00:00')
    );
    ...
    cy.get('@nameInfoTable')
      .contains('Tulevaisuuden yliopisto')
      .should('not.exist');
    cy.get('@displayNameInfoTable')
      .contains('Menneisyyden yliopisto (HY)')
      .should('exist');
    cy.get('@displayNameInfoTable')
      .contains('Helsingin yliopisto (HY)')
      .should('exist');
    cy.get('@displayNameInfoTable')
      .contains('Tulevaisuuden yliopisto (HY)')
      .should('not.exist');
    ...
    cy.get('@showHistorySwitch').click();
    cy.get('@nameInfoTable')
      .contains('Menneisyyden yliopisto')
      .should('not.exist');
    cy.get('@nameInfoTable')
      .contains('Tulevaisuuden yliopisto')
      .should('not.exist');
  ...
  it('shows correct information when unit is past in relation to selected day', () => {
    cy.intercept(
      'GET',
      /.*\/api\/node\/[0-9]+\/[0-9]+\.[0-9]+\.[0-9]+\/attributes/g,
      { fixture: '2022-06-21/past/attributes.json' }
    );
    cy.intercept(
      'GET',
      /.*\/api\/node\/parents\/[0-9]+\/[0-9]+\.[0-9]+\.[0-9]+/g,
      { fixture: '2022-06-21/past/parents.json' }
    );
    cy.intercept(
      'GET',
      /.*\/api\/node\/children\/[0-9]+\/[0-9]+\.[0-9]+\.[0-9]+/g,
      { fixture: '2022-06-21/past/children.json' }
    );
    cy.intercept(
      'GET',
      /.*\/api\/node\/predecessors\/[0-9]+\/[0-9]+\.[0-9]+\.[0-9]+/g,
      { fixture: '2022-06-21/past/predecessors.json' }
    );
    cy.intercept(
      'GET',
      /.*\/api\/node\/successors\/[0-9]+\/[0-9]+\.[0-9]+\.[0-9]+/g,
      { fixture: '2022-06-21/past/successors.json' }
    );
    cy.intercept(
      'GET',
      /.*\/api\/node\/historyandcurrent\/[0-9]+\/[0-9]+\.[0-9]+\.[0-9]+\/attributes/g,
      { fixture: '2022-06-21/past/historyAttributes.json' }
    );
    cy.intercept(
      'GET',
      /.*\/api\/node\/historyandcurrent\/parents\/[0-9]+\/[0-9]+\.[0-9]+\.[0-9]+/g,
      { fixture: '2022-06-21/past/historyParents.json' }
    );
    cy.intercept(
      'GET',
      /.*\/api\/node\/historyandcurrent\/children\/[0-9]+\/[0-9]+\.[0-9]+\.[0-9]+/g,
      { fixture: '2022-06-21/past/historyChildren.json' }
    );
    cy.intercept(
      'GET',
      /.*\/api\/node\/futureandcurrent\/[0-9]+\/[0-9]+\.[0-9]+\.[0-9]+\/attributes/g,
      { fixture: '2022-06-21/past/futureAttributes.json' }
    );
    cy.intercept(
      'GET',
      /.*\/api\/node\/futureandcurrent\/parents\/[0-9]+\/[0-9]+\.[0-9]+\.[0-9]+/g,
      { fixture: '2022-06-21/past/futureParents.json' }
    );
    cy.intercept(
      'GET',
      /.*\/api\/node\/futureandcurrent\/children\/[0-9]+\/[0-9]+\.[0-9]+\.[0-9]+/g,
      { fixture: '2022-06-21/past/futureChildren.json' }
    );
  ...
  it('shows correct information when unit is future in relation to selected day', () => {
    cy.intercept(
      'GET',
      /.*\/api\/node\/[0-9]+\/[0-9]+\.[0-9]+\.[0-9]+\/attributes/g,
      { fixture: '2022-06-21/future/attributes.json' }
    );
    cy.intercept(
      'GET',
      /.*\/api\/node\/parents\/[0-9]+\/[0-9]+\.[0-9]+\.[0-9]+/g,
      { fixture: '2022-06-21/future/parents.json' }
    );
    cy.intercept(
      'GET',
      /.*\/api\/node\/children\/[0-9]+\/[0-9]+\.[0-9]+\.[0-9]+/g,
      { fixture: '2022-06-21/future/children.json' }
    );
    cy.intercept(
      'GET',
      /.*\/api\/node\/predecessors\/[0-9]+\/[0-9]+\.[0-9]+\.[0-9]+/g,
      { fixture: '2022-06-21/future/predecessors.json' }
    );
    cy.intercept(
      'GET',
      /.*\/api\/node\/successors\/[0-9]+\/[0-9]+\.[0-9]+\.[0-9]+/g,
      { fixture: '2022-06-21/future/successors.json' }
    );
    cy.intercept(
      'GET',
      /.*\/api\/node\/historyandcurrent\/[0-9]+\/[0-9]+\.[0-9]+\.[0-9]+\/attributes/g,
      { fixture: '2022-06-21/future/historyAttributes.json' }
    );
    cy.intercept(
      'GET',
      /.*\/api\/node\/historyandcurrent\/parents\/[0-9]+\/[0-9]+\.[0-9]+\.[0-9]+/g,
      { fixture: '2022-06-21/future/historyParents.json' }
    );
    cy.intercept(
      'GET',
      /.*\/api\/node\/historyandcurrent\/children\/[0-9]+\/[0-9]+\.[0-9]+\.[0-9]+/g,
      { fixture: '2022-06-21/future/historyChildren.json' }
    );
    cy.intercept(
      'GET',
      /.*\/api\/node\/futureandcurrent\/[0-9]+\/[0-9]+\.[0-9]+\.[0-9]+\/attributes/g,
      { fixture: '2022-06-21/future/futureAttributes.json' }
    );
    cy.intercept(
      'GET',
      /.*\/api\/node\/futureandcurrent\/parents\/[0-9]+\/[0-9]+\.[0-9]+\.[0-9]+/g,
      { fixture: '2022-06-21/future/futureParents.json' }
    );
    cy.intercept(
      'GET',
      /.*\/api\/node\/futureandcurrent\/children\/[0-9]+\/[0-9]+\.[0-9]+\.[0-9]+/g,
      { fixture: '2022-06-21/future/futureChildren.json' }
    );
