            ...
            children: [
              {
                ...
                displayNameFi:
                  'HY-TRI, Humanistis-yhteiskuntatieteellinen tutkijakoulu (HYMY)',
                displayNameEn:
                  'HY-TRI, Doctoral School in the Humanities and Social Sciences (HYMY)',
                displayNameSv:
                  'HY-TRI, Humanistisk-samhällsvetenskapliga forskarskolan (HYMY)',
                type: 'tutkijakoulu',
                abbreviation: 'HYMY',
...
const historyNodes = [
  ...
  },
  {
    ...
    hierarchies: ['opetus', 'talous', 'tutkimus', 'henkilosto'],
  },
];
...
test('flattenTree result is not larger than original tree', () => {
  ...
  const flatSize = encodeURI(
    JSON.stringify(flatTreee).split(/%..|./).length - 1
  );
  const treeSize = encodeURI(
    JSON.stringify(inputTree).split(/%..|./).length - 1
  );
...
test('Attribute duplicates are filtered', () => {
  ...
  expect(
    filterAttributeDuplicates(historyAttributes, futureAttributes).length
  ).toEqual(6);
});
