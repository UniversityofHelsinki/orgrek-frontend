  ...
  return async (dispatch) => {
    ...
    if (response.status === 200) {
      dispatch({
        ...
        payload: {
          message: 'update_parent_unit_properties_success',
          success: true,
        },
      });
      setTimeout(() => {
      ...
      dispatch({
        type: 'SHOW_NOTIFICATION',
        payload: {
          message: 'update_parent_unit_properties_error',
          success: false,
  ...
  return async (dispatch) => {
    ...
    if (response.status === 200) {
      dispatch({
        type: 'SHOW_NOTIFICATION',
        payload: {
          message: 'update_attributes_success',
          success: true,
      ...
      dispatch({
        type: 'SHOW_NOTIFICATION',
        payload: {
          message: 'update_attributes_error',
          success: false,
          statusCode: response.status,
          skipValidation: skipValidation,
        },
  ...
  return async (dispatch) => {
    ...
    if (response.status === 200) {
      ...
      dispatch({
        type: 'SHOW_NOTIFICATION',
        payload: {
          message: 'update_properties_error',
          success: false,
          statusCode: response.status,
        },
      });
