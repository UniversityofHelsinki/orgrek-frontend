...
const NodeDetails = (props) => {
  ...
  const nameInfoData = attributeData
    ? attributeData.filter((elem) => /^name_(fi|sv|en)$/.test(elem.key))
    : false;
  const nameInfoDataOrderedByLanguage = nameInfoData
    ? orderNameAttributesByLanguage(nameInfoData)
    : false;
  const typeAttributeData = attributeData
    ? attributeData.filter((elem) => elem.key === 'type')
    : false;
  const codeAttributesData = attributeData
    ? [
        uniqueIdAttribute,
        ...attributeData.filter((a) => codeAttributes.includes(a.key)),
      ]
        .sort(byCodesAndDates)
        .filter(
          (a) => !nameInfoData.includes(a) && !typeAttributeData.includes(a)
        )
    : false;
  const otherAttributesData = attributeData
    ? attributeData.filter((elem) => {
        return (
          !nameInfoData.includes(elem) &&
          !typeAttributeData.includes(elem) &&
          !codeAttributesData.includes(elem)
        );
      })
    : false;
  const sortedOtherAttributesData = otherAttributesData
    ? sortOtherAttributes(otherAttributesData)
    : false;
  ...
  const isPast =
    !isCurrent &&
    props.node?.endDate &&
    new Date(Date.parse(props.node.endDate)).getTime() <=
      props.selectedDay.getTime();
  const isFuture =
    !isCurrent &&
    props.node?.startDate &&
    new Date(Date.parse(props.node.startDate)).getTime() >=
      props.selectedDay.getTime();
  ...
  useEffect(() => {
    if (props.node) {
      ...
      if (
        ...
        endDate &&
        new Date(endDate).getTime() <= props.selectedDay.getTime()
      ) {
        props.fetchNodeDetails(
          props.node,
          new Date(endDate),
        ...
        startDate &&
        new Date(startDate).getTime() >= props.selectedDay.getTime()
      ) {
        props.fetchNodeDetails(
          props.node,
          new Date(startDate),
  ...
  const onValueChange = (event, elem) => {
    if (modified[elem.id]) {
      //element has already been modified at least once, because its found in modified array
      const target = {
        ...modified[elem.id],
        [event.target.name]: event.target.value,
        validity: false,
        err: '',
      }; //makes copy of modified[elem.id] and updates its value with event.target.value
      setModified({ ...modified, [elem.id]: target }); //updates row
    } else {
      //This is the first time this element is modified so its not found in modified array
      const target = {
        ...elem,
        [event.target.name]: event.target.value,
        validity: false,
        err: '',
      }; //creates a new elem object based on elem object and updates its value with event.target.value
  ...
  const onDateChange = (dateChanged) => {
    ...
    let date = moment(dateChanged.date)
      .utcOffset(0)
    ...
    if (modified[elem.id]) {
      ...
      const target = {
        ...modified[elem.id],
        [name]: date,
        validity: dateChanged.validity,
        err: '',
      }; //makes copy of modified[elem.id] and updates its value with date
      setModified({ ...modified, [elem.id]: target }); //updates row
    } else {
      //This is the first time this element is modified so its not found in modified array
      const target = {
        ...
        validity: dateChanged.validity,
        err: '',
      }; //creates a new elem object based on elem object and updates its value with date
  ...
  const onParentDateChange = (dateChanged) => {
    ...
    let hierarchyIndex = elem.hierarchies.findIndex(
      (obj) => obj.hierarchy === dateChanged.hierarchy
    );
    if (name === 'startDate') {
        ...
        <>
          ...
          <div className="right-side">
            <div>
              <h3>
                {
                  props.favorableNames[(lang === 'ia' && 'fi') || lang]?.[0]
                    ?.name
                }
              </h3>
            </div>
            ...
            <NodeDetailsTable
              ...
              contentData={
                nameInfoDataOrderedByLanguage
                  ? [
                      ...nameInfoDataOrderedByLanguage.map((elem) => {
                        if (modified[elem.id]) {
                          return modified[elem.id];
                        }
                        return elem;
            ...
            />
            <NewAttribute
              availableAttributes={availableNames}
            ...
            <NodeDetailsTable
              ...
              tableLabels={['text_language_header', 'name']}
              contentData={[
                ...(props.displayNames.fi || []),
                ...(props.displayNames.sv || []),
                ...(props.displayNames.en || []),
              ]
                .filter((n) => n)
                .map((dn) => ({
                  ...dn,
            ...
            <NodeDetailsTable
              ...
              hasValidity={true}
              dataFilter={(data) =>
                (isPast || isFuture) && !(props.showHistory || props.showComing)
                  ? data.filter((attr) => attr.key === 'unique_id')
                  : data
              }
              onValueChange={onValueChange}
              onDateChange={onDateChange}
            ...
            />
            <NewAttribute
              availableAttributes={availablecodes}
              initvalues={initValues}
            />
            <NodeDetailsTable
              selectedDay={props.selectedDay}
            ...
            <NodeDetailsTable
              ...
              contentData={
                sortedOtherAttributesData
                  ? [
                      ...sortedOtherAttributesData.map((elem) => {
                        if (modified[elem.id]) {
                          return modified[elem.id];
                        }
                        return elem;
