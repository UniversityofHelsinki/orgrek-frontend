...
const EditButtons = (props) => {
    ...
    const saveModifiedAttributes = async()  => {
        ...
        if (filteredAttributesArray && filteredAttributesArray.length > 0) {
            await props.updatingAttributes(props.node, filteredAttributesArray, skipValidation);
        }
    ...
    useEffect( async () => {
        ...
        if (props.feedback && props.feedback.success && props.feedback.message !== 'insert_attributes_success') {
            toggleEdit(false);
