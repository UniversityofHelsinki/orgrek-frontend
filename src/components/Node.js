...
const Node = (props) => {
    ...
    useEffect(() => {
        if (props.item?.hierarchies) {
            setShowHierarchies(!containsAll(props.item.hierarchies, props.selectedHierarchy.split(',')));
        }
    }, [props.item]);
        ...
        <div style={{ display: 'flex', cursor:'pointer', paddingLeft: `${props.level * 35}px` }}>
             {props.hasChildren
                ...
                <div className="treeNode">
                    <span className={props.node && props.node.uniqueId === props.item.uniqueId ? 'nodeLinkSelected' : 'nodeLink'}
                    ...
                    {showHierarchies && <span className="treeHierarchies">{props.item?.hierarchies.map(h => t(h)).reduce((a,c) => `${a} ${c}`)}</span>}
                </div>
        </div>
    );
};
