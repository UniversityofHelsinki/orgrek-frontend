  ...
  return (
    <>
      <InputGroup>
        ...
        <Form.Control
          placeholder={t('texts_search_placeholder')}
          value={query}
  ...
  return (
    <>
      <tr>
        ...
        <th>
          <span onClick={onClick('language')} className="boldHeader">
            {t('texts_language')}
          </span>
          {arrow === 'language' && (
            <span className="littleLeftMargin">
              {scendings['language'] === 1 ? '\u25bd' : '\u25b3'}
            </span>
          )}
        </th>
        <th>
        ...
        <th>
          <span onClick={onClick('user_name')} className="boldHeader">
            {t('texts_editor')}
          </span>
          {arrow === 'user_name' && (
            <span className="littleLeftMargin">
              {scendings['user_name'] === 1 ? '\u25bd' : '\u25b3'}
            </span>
          )}
        </th>
        <th>
          <span onClick={onClick('timestamp')} className="boldHeader">
            {t('texts_timestamp')}
          </span>
          {arrow === 'timestamp' && (
            <span className="littleLeftMargin">
              {scendings['timestamp'] === 1 ? '\u25bd' : '\u25b3'}
            </span>
          )}
        </th>
        <th>
  ...
  useEffect(() => {
    ...
    if (awaitingDeletionFeedback) {
      ...
      if (props.feedback.success) {
        ...
        props.setTexts([
          ...props.texts.filter(
            (t) => !(t.key === text.key && t.language === text.language)
          ),
        ]);
      }
    }
        ...
        {edit ? (
          <>
            <Form.Control
              ...
              rows={3}
              value={value}
              onChange={(e) => e.target.value.length <= 255 && onValueChange(e)}
            />
            <span>
              {255 - value.length} {t('texts_value_characters_left')}
            </span>
          </>
        ) : (
        ...
        {edit ? (
          <ButtonGroup vertical>
            ...
            <Button
              ...
              variant="success"
              onClick={save}
              disabled={saveInProgress || value === text.value}
            >
              {saveInProgress ? (
                <>
                  <Spinner
...
const FormRow = (props) => {
  ...
  const isValid = (text) => {
    return (
      text.key &&
      !alreadyExists(text.key, text.language) &&
      text.value?.length > 0 &&
  ...
  const alreadyExists = (key, language) => {
    return (
      key &&
      props.texts?.some(
        (t) => t.key === key.toLowerCase() && t.language === language
      )
    );
  };
  ...
  return (
    <tr>
      <td>
        <InputGroup>
          <Form.Control
            placeholder={t('texts_key')}
            value={text.key || ''}
            onChange={(e) => onChange('key', e.target.value)}
          />
        </InputGroup>
        {props.alreadyExists(text.key, text.language) && (
          <span className="warningText">{t('texts_key_already_exists')}</span>
        )}
      </td>
      <td>
        <DropdownButton title={text.language || t('texts_language')}>
          <BSDropdown.Item onClick={() => onChange('language', 'fi')}>
            {t('fi')}
          </BSDropdown.Item>
          <BSDropdown.Item onClick={() => onChange('language', 'sv')}>
            {t('sv')}
          </BSDropdown.Item>
          <BSDropdown.Item onClick={() => onChange('language', 'en')}>
            {t('en')}
          </BSDropdown.Item>
        </DropdownButton>
        {props.showLanguageWarning && !text.language && (
          <span className="warningText">{t('texts_language_missing')}</span>
        )}
      </td>
      <td>
        <InputGroup>
          <Form.Control
            as="textarea"
            rows={1}
            placeholder={t('texts_value')}
            value={text.value || ''}
            onChange={(e) =>
              e.target.value.length <= 255 && onChange('value', e.target.value)
            }
          />
        </InputGroup>
        {text.value?.length > 0 && (
          <span>
            {255 - text.value.length} {t('texts_value_characters_left')}
          </span>
        )}
      </td>
      <td>{text.user_name}</td>
...
const Texts = (props) => {
  ...
  useEffect(() => {
    if (awaitingFeedback) {
      ...
      if (props.feedback.success && props.feedback.texts) {
        ...
        props.setTexts([...props.texts, ...props.feedback.texts]);
      }
    }
  }, [props.feedback]);
  ...
  useEffect(() => {
    setFormRows(
      formRows.map((row) => ({ ...row, user_name: props.user.eppn }))
    );
  }, [props.user]);
  ...
  useEffect(() => {
    const currentTexts = props.texts.map((t) => {
      const found = updatedTexts.find(
        (ut) => ut.key === t.key && ut.language === t.language
      );
      return found || t;
    });
    if (searchQuery && searchQuery.length > 0) {
      setTexts(
        currentTexts.filter((text) => {
  ...
  const alreadyExists = (key, language) => {
    return (
      ...
      (props.texts?.some(
        ...
        formRows
          .filter((r) => r)
          .filter((t) => t.key === key && t.language === language).length > 1)
    );
  };
  ...
  const createMissingLanguages = (text) => {
    const existingLanguages = [
      ...props.texts.filter((t) => t.key === text.key),
      ...formRows.filter((r) => r && r.key === text.key),
    ];
    const missingLanguages = ['fi', 'sv', 'en'].filter(
      (lang) => !existingLanguages.some((t) => t.language === lang)
    );
    return missingLanguages.map((lang) => ({
      ...text,
      value: text.key,
      language: lang,
    }));
  ...
  const saveNewTexts = () => {
    ...
    props.insertTexts(
      formRows
        ...
        .reduce(
          (prev, curr) =>
            prev.some((t) => t.key === curr.key)
              ? [...prev, curr]
              : [...prev, curr, ...createMissingLanguages(curr)],
          []
        )
    );
  };
  ...
  const onTextUpdated = (updatedText) => {
    const duplicateRemoved = updatedTexts.filter(
      (text) =>
        text.key !== updatedText.key && text.language !== updatedText.language
    );
        ...
        <Col md="auto">
          <BSRow className="justify-content-end">
            <Col md="auto">
              <Button
                onClick={() =>
                  setFormRows([...formRows, { user_name: props.user.eppn }])
                }
                disabled={saveNewTextsInProgress}
            ...
            <Col
              md="auto"
              onClick={
                !areNewTextsValid || saveNewTextsInProgress
                  ? () => setShowLanguageWarning(true)
                  : () => {}
              }
            >
              <Button
                onClick={saveNewTexts}
                disabled={!areNewTextsValid || saveNewTextsInProgress}
                variant="success"
              >
                {saveNewTextsInProgress ? (
                  <>
                    ...
                    />
                    <span style={{ marginLeft: '10px' }}>
                      {t('texts_save_new_rows_in_progress')}
                    </span>
                  </>
                ) : (
